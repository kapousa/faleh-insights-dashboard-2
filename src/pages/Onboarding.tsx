...
const Onboarding = () => {
    ...
    return (
        <div className="min-h-screen bg-[#fcfcfd] flex flex-col md:flex-row overflow-hidden font-sans">
            ...
            <div
                className="w-full md:w-[380px] bg-[#0a1d37] p-10 text-white flex flex-col justify-between relative overflow-hidden shrink-0">
                {/* لمسة جمالية للخلفية */}
                <div className="absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none"
                     style={{
                         backgroundImage: 'radial-gradient(#5c21ff 2px, transparent 2px)',
    ...
    />
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-4 pt-4 border-l border-white/10 ml-3 pl-6">
                            <div
                                className={`relative flex items-center gap-3 transition-opacity ${phaseIndex >= -1 ? "opacity-100" : "opacity-30"}`}>
                                <div
                                    className={`absolute -left-[31px] w-4 h-4 rounded-full border-4 ${phaseIndex >= -1 ? "bg-[#ffcc00] border-[#0a1d37]" : "bg-slate-700 border-[#0a1d37]"}`}/>
                                <span
                                    className="text-[10px] font-black uppercase italic tracking-wider">Business Info</span>
                            </div>
                            {ASSESSMENT_PHASES.map((phase, idx) => (
                                <div key={idx}
                                     className={`relative flex items-center gap-3 transition-opacity ${phaseIndex >= idx ? "opacity-100" : "opacity-30"}`}>
                                    <div
                                        className={`absolute -left-[31px] w-4 h-4 rounded-full border-4 ${phaseIndex >= idx ? "bg-[#ffcc00] border-[#0a1d37]" : "bg-slate-700 border-[#0a1d37]"}`}/>
                                    <span
                                        className="text-[10px] font-black uppercase italic tracking-wider">{phase.title}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="max-w-2xl w-full mx-auto px-8 py-16">
                    ...
                    <div className="mb-12 flex items-center justify-between border-b border-slate-50 pb-8">
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Franchise
                                Readiness Audit</p>
                        </div>
                        ...
                        {phaseIndex === -1 ? (
                            ...
                            >
                                <div className="space-y-2">
                                    ...
                                    <h2 className="text-4xl font-black uppercase italic text-[#0a1d37] leading-none">Who
                                        are we <br/><span className="text-[#5c21ff]">auditing?</span></h2>
                                </div>

                                <div className="grid gap-8">
                                    <div className="space-y-3">
                                        <Label
                                            className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Legal
                                            Entity Name</Label>
                                        <Input
                                            className="h-16 rounded-none border-2 border-slate-100 bg-slate-50/30 text-lg font-bold focus:bg-white transition-all shadow-sm"
                                            value={businessName} onChange={(e) => setBusinessName(e.target.value)}
                                            placeholder="e.g. Faleh Systems LLC"/>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-3">
                                            <Label
                                                className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Primary
                                                Contact</Label>
                                            <Input
                                                className="h-16 rounded-none border-2 border-slate-100 bg-slate-50/30 font-bold"
                                                value={contactName} onChange={(e) => setContactName(e.target.value)}
                                                placeholder="Full Name"/>
                                        </div>
                                        <div className="space-y-3">
                                            <Label
                                                className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Business
                                                Email</Label>
                                            <Input
                                                className="h-16 rounded-none border-2 border-slate-100 bg-slate-50/30 font-bold"
                            ...
                            >
                                <div className="p-8 bg-[#0a1d37] text-white relative overflow-hidden group">
                                    <div
                                        className="absolute top-0 right-0 w-32 h-32 bg-[#5c21ff] opacity-10 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform"/>
                                    ...
                                    {currentPhase?.questions.map((q) => (
                                        <div key={q.id} className="space-y-6">
                                            <div className="flex gap-4">
                                                ...
                                                <Label
                                                    className="text-xl font-black italic text-[#0a1d37] leading-tight">
                                                    {q.question}
                                                </Label>
                                            </div>
                                ...
                                <span
                                    className={`font-black uppercase italic text-xs tracking-wider ${answers[q.id] === opt.value ? "text-[#0a1d37]" : "text-slate-400 group-hover:text-slate-500"}`}>
                                  {opt.label}
                                </span>
                                                                {answers[q.id] === opt.value && (
                                                                    <motion.div initial={{scale: 0}}
                                                                                animate={{scale: 1}}>
                                                                        <ShieldCheck className="text-[#ffcc00]"
                                                                                     size={20}/>
                                                                    </motion.div>
                                                                )}
                                                            </div>
